...
class Testcentripetal_force_tang_vel(unittest.TestCase):
    """
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.centripetal_force_tang_vel(
                        mass=mass[i], radius=radius[i], velocity=velocity[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.centripetal_force_tang_vel(
                    mass=mass[i], radius=radius[i], velocity=velocity[i]
                )
    ...
    def test_solving_for_velocity(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.centripetal_force_tang_vel(
                        centripetal_F=centripetal_F[i], radius=radius[i], mass=mass[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.centripetal_force_tang_vel(
    ...
    def test_solving_for_radius(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                ...
                result = Chapter6.Calculate.centripetal_force_tang_vel(
                    centripetal_F=centripetal_F[i], velocity=velocity[i], mass=mass[i]
                )
                self.assertAlmostEqual(result, expected[i], places=2)


class Testcentripetal_force_ang_vel(unittest.TestCase):
    """
    Tests the main use cases of calculations for the centripetal acceleration
    ...
    def test_solving_for_centripetal_F(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.centripetal_force_ang_vel(
                        radius=radius[i], angular_vel=angular_vel[i], mass=mass[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.centripetal_force_ang_vel(
                    radius=radius[i], angular_vel=angular_vel[i], mass=mass[i]
                )
                self.assertAlmostEqual(result, expected[i], places=2)
    ...
    def test_solving_for_angular_vel(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.centripetal_force_ang_vel(
                        radius=radius[i], mass=mass[i], centripetal_F=centripetal_F[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.centripetal_force_ang_vel(
...
class Testideal_ang_banked_curve(unittest.TestCase):
    """
    ...
    def test_solving_for_theta(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.ideal_ang_banked_curve(
                        velocity=velocity[i], radius=radius[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.ideal_ang_banked_curve(
                    velocity=velocity[i], radius=radius[i]
                )
                self.assertAlmostEqual(result, expected[i], places=2)
    ...
    def test_solving_for_radius(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.ideal_ang_banked_curve(
                        theta=theta[i], velocity=velocity[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.ideal_ang_banked_curve(
...
class Testdrag_force(unittest.TestCase):
    """
    ...
    def test_solving_for_drag_F(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                ...
                result = Chapter6.Calculate.drag_force(
                    drag_coeff=drag_coeff[i],
                    fluid_dens=fluid_dens[i],
                    area=area[i],
                    velocity=velocity[i],
...
class Teststokes_law(unittest.TestCase):
    """
    ...
    def test_solving_for_drag_Fs(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.stokes_law(
                        radius=radius[i], viscosity=viscosity[i], velocity=velocity[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.stokes_law(
                    radius=radius[i], viscosity=viscosity[i], velocity=velocity[i]
                )
                self.assertAlmostEqual(result, expected[i], places=2)
    ...
    def test_solving_for_radius(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.stokes_law(
                        drag_Fs=drag_Fs[i], viscosity=viscosity[i], velocity=velocity[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.stokes_law(
    ...
    def test_solving_for_viscosity(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.stokes_law(
                        drag_Fs=drag_Fs[i], radius=radius[i], velocity=velocity[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.stokes_law(
    ...
    def test_solving_for_velocity(self) -> None:
        ...
        radius: List[float] = [0.25, 0.25, 0.25]
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.stokes_law(
                        drag_Fs=drag_Fs[i], radius=radius[i], viscosity=viscosity[i]
                    )
                self.assertEqual(str(context.exception), str(expected[i]))
            else:
                result = Chapter6.Calculate.stokes_law(
...
class Testterminal_velocity(unittest.TestCase):
    """
    ...
    def test_solving_for_terminal_vel(self) -> None:
        ...
        for i in range(len(expected)):
            if isinstance(expected[i], ValueError):
                with self.assertRaises(ValueError) as context:
                    Chapter6.Calculate.terminal_velocity(
                        mass=mass[i],
                        drag_coeff=drag_coeff[i],
                        area=area[i],
                        fluid_dens=fluid_dens[i],
                ...
                result = Chapter6.Calculate.terminal_velocity(
                    mass=mass[i],
                    drag_coeff=drag_coeff[i],
                    area=area[i],
                    fluid_dens=fluid_dens[i],
